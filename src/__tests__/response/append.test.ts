...
describe('ctx.append(name, val)', () => {
  it('should append multiple headers', () => {
    ...
    expect(ctx.response.header['x-foo']).toStrictEqual(['bar1', 'bar2']);
  });
  ...
  it('should accept array of values', () => {
    ...
    expect(ctx.response.header['set-cookie']).toStrictEqual(['foo=bar', 'fizz=buzz', 'hi=again']);
  });
  ...
  it('should work with res.set(field, val) first', () => {
    ...
    expect(ctx.response.header.link).toStrictEqual(['<http://localhost/>', '<http://localhost:80/>']);
  });
});
